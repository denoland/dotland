...
function Registry(): React.ReactElement {
  ...
  const sourceURL = useMemo(
    () => getSourceURL(name, version, path),
    [name, version, path]
  );
  ...
  const { readmeCanonicalPath, readmeURL, readmeRepositoryURL } =
    useMemo(() => {
      const readmeEntry =
        path === ""
          ...
          <div className="mt-8">
            {(() => {
              if (versions === null) {
                return (
                  <ErrorMessage
                    title="404 - نەدۆزرایەوە"
                    body="ئەم مۆدیوڵە بوونی نییە."
                  />
                );
              } else if (
                ...
                return (
                  <ErrorMessage
                    title="هیچ وەشانێکی بەردەست نییە"
                    body={`ئەم مۆدیوڵە ناوەکەی گیراوە، بەڵام هیچ وەشانێکی بڵاو نەکراوەتەوە. ئەو مۆدیوڵانەی تا 30 ڕۆژ دوای تۆمار بوونیان هیچ وەشانێکیان بڵاو ناکرێتەوە، خۆکارانە لادەبرێن. ${
                      versions.isLegacy
                        ? "ئەگەر تۆ خاوەنی ئەم مۆدیوڵەیت، تکایە سەرچاوەکەی دیسانەوە زۆر بکە لە ڕێی deno.land/x (بە ڕەچاوکردنی هەنگاوەکان لە https://deno.land/x#add) و وەشانێکی نوێ بڵاو بکەرەوە."
                        : ""
                    }`}
                  />
                );
