...
/** @jsx h */
/** @jsxFrag Fragment */
import { Fragment, h, useData, useEffect, useMemo, useState } from "../deps.ts";
...
export function RegistryInstructions(props: {
  isOpen: boolean;
  close: () => void;
}) {
  ...
  const isModuleNameAvailable = useData(
    moduleName,
    (moduleName) =>
                ...
                <div className="space-y-4">
                  {stage === 0 && (
                    <>
                      ...
                      <p className="text-base">
                        <b className="font-semibold">deno.land/x</b>{" "}
                        downloads and stores your repository contents every time
                        you create a git tag. We only do this once for every
                        tag. This ensures that the contents we serve for a
                        specific version can never change.
                      </p>
                      <p className="text-base">
                        ...
                        {!isSubdirectoryValid
                          ? (
                            <p className="text-red-400 mb-2">
                              ...
                              <InlineCode>/</InlineCode>, but may not start with
                              one. (e.g. <InlineCode>src/</InlineCode>)
                            </p>
                          )
                          : null}
