...
function serializeSignature(
  ...
  if (!signature.declaration) {
    return ({
      missing: "signature.declaration"
    } as unknown) as SignatureDocNode;
  }
  return {
...
function visit(
  ...
  if (
    ...
    console.log(`unhandled doc node: ${node.kind} ${ts.SyntaxKind[node.kind]}`);
  }
}
...
export function main(rootModule: string, rootSource: string): ModuleDocNode[] {
  const fileNames: string[] = [rootModule];
  const host = new Host(rootModule, rootSource);
  const options = {}; //host.getCompilationSettings();
  const program = ts.createProgram(fileNames, options, host);
  return generateDoc(program, rootModule);
}
