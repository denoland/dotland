...
presentation Design Mistakes in Node, this is discussed in more detail. Due to
the large number of users that Node has, it is difficult and slow to evolve the
system.
...
are all found inside the "Deno" namespace (EG `Deno.open()`). These have been
...
In the global namespace you'll find all sorts of other objects (EG
...
Deno's HTTP server is implemented in JavaScript on top of native TCP sockets.
Node's HTTP server is in C and exposed as high-level bindings to JavaScript. We
have resisted the urge to add native HTTP server bindings to Deno, because we
want to optimize the TCP socket layer, and more generally the op interface.
...
[@bartlomieju](https://github.com/bartlomieju) has touched every part of the
system and his contributions are too numerous to list but most notably: he wrote
the deno dir system, deno doc, import maps, web workers, Deno.watchFs, and the
CLI flag parsing. [@kitsonk](https://github.com/kitsonk) has had a massive hand
in many parts of the system, including but not limited to the TypeScript
compiler host, deno_typescript, deno bundle, deno install, deno types, streams
implementation. [@kevinkassimo](https://github.com/kevinkassimo) has contributed
countless bug fixes over the whole history of the project. Among his
contributions are the timer system, TTY integration, wasm support.
...
[@kt3k](https://github.com/kt3k) implemented the continuous benchmark system
(which has been instrumental in almost every major refactor), signal handlers,
the permissions API, and many critical bug fixes.
[@nayeemrmn](https://github.com/nayeemrmn) contributes bug fixes in many parts
of Deno, most notably he greatly improved the stack trace and error reporting,
and has been a forceful help towards the stabilizing the APIs for 1.0.
[@justjavac](https://github.com/justjavac) has contributed many small but
critical fixes to align deno APIs with web standards and most famously he wrote
the VS Code deno plugin. [@zekth](https://github.com/zekth) has contributed a
lot of modules to std, among them the std/encoding/csv, std/encoding/toml,
std/http/cookies, as well as many other bug fixes.
...
[And many others](https://github.com/denoland/deno/graphs/contributors).
