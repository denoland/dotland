今天我们将发布 Deno 1.8.0。此版本包含大量新功能和稳定性改进：

- [**对 WebGPU API 的实验支持**](#experimental-support-for-the-webgpu-api):
  paving a path towards out-of-the-box GPU accelerated machine learning in Deno
- [**启用了内置的国际化 APIs 支持**](#icu-support): all JS `Intl` APIs are
...
## Revamped coverage tooling: `deno coverage` {#revamped-coverage-tooling-codedeno-coveragecode}
...
## Import maps are now stable {#import-maps-are-now-stable}
...
## Auth token support for fetching modules {#auth-token-support-for-fetching-modules}
...
## Exit sanitizer for `Deno.test` {#exit-sanitizer-for-codedenotestcode}
...
## `Deno.permissions` APIs are now stable {#codedenopermissionscode-apis-are-now-stable}
...
## `Deno.link` and `Deno.symlink` APIs have been stabilized {#codedenolinkcode-and-codedenosymlinkcode-apis-have-been-stabilized}
...
## More granular `Deno.metrics` {#more-granular-codedenometricscode}
...
## JSON support for `deno fmt` {#json-support-for-codedeno-fmtcode}
...
## IIFE bundle support for `Deno.emit` {#iife-bundle-support-for-codedenoemitcode}
...
## `deno lsp` is now stable {#codedeno-lspcode-is-now-stable}
