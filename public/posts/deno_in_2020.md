...
“rusty_v8” is a Rust crate that provides Rust API for V8 engine (link). By
December “rusty_v8” had all required bindings to replace “libdeno”. The effort
started at the end of 2019, where the first parts of “libdeno” were rewritten
using “rusty_v8”. Thanks to a lot of tests in the Deno codebase we were
confident moving forward and wrapped up the effort in a fortnight. “libdeno” was
completely removed in release 0.29.0 and since then “rusty_v8” has undergone
major refactors to type safety of the bindings.
...
We got introduced to “dprint” by David Sherret, a code formatter written in
Rust. “dprint” could format the code the same way the “prettier” module did but
it was orders of magnitude faster. After some preliminary testing we decided to
“dprint” in “deno fmt”.
...
Two new subcommands were added to the CLI: “deno doc” “deno upgrade” We also saw
a huge improvement to the build process. Up to this point, each build of the
“deno” V8 engine was built from source. V8 is a huge C++ project that can easily
take 30 minutes to build. Thanks to the changes in rusty_v8 and the deno build
process we were able to cut compile times to a couple minutes, speeding up the
...
This month marked the last 0.x.x release of Deno.
...
Before 1.0 release removed some features: Remove features like: JSON imports,
WASM imports, window.location Drop “deno script.ts” shortcut Remove Rust API for
“deno” crate - only binary

1.0 release exactly 2 years afterRyan’s presentation
...
**Releases that month:** 1.2.3, 1.3.0, 1.3.1, 1.3.2
...
Deno_crypto op crate Simplify tooling in the repo
