...
is the official VSCode extension for Deno. It implements a lot of functionality
by duplicating fundamental behaviours of Deno; a stark example is the module
resolution logic that requires module specifiers with extensions, and support
for import maps. Keeping extension logic in sync with Deno logic posed a
significant challenge. In addition, providing high quality IDE experiences in an
editor other than VSCode would require us to duplicate these efforts all over.
Providing a self-contained LSP implementation in Deno makes it easier to provide
a high-quality and unified editing experience for all code editors that
understand LSP (VSCode, `vim`, `emacs`, Atom, and many others).

The LSP is not yet feature-complete, but it implements many of the main LSP
functionalities:
...
Because we do not yet have feature parity with the current VSCode extension, we
are not yet ready to replace it. To let you try out this new experience anyway,
we have published
[VSCode Deno Canary](https://marketplace.visualstudio.com/items?itemName=denoland.vscode-deno-canary)
to the VSCode Marketplace. This is a new version of VSCode extension that uses
`deno lsp`. Make sure that you have installed Deno 1.6 before trying out the new
extension.
...
For more information on new features in Typescript 4.1 visit the official
