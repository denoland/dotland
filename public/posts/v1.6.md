...
## Experimental support for Apple's new M1 Arm64 CPU

Users of the new Apple computers will be able to run Deno natively.
We refer to this target by the LLVM target triple `aarch64-apple-darwin` in our
release zip files.

This target is still considered experimental because they have been built using
rusty nightly (we normally use rust stable) and because we do not yet have
...
[Binaries of `rusty_v8 v0.14.0` targetting M1 are also
provided](https://github.com/denoland/rusty_v8/releases/tag/v0.14.0) with the
same caveats.
