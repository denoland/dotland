...
Which later can be imported as such:
...
`deno fmt` now supports formatting Markdown files, including formatting of JS/TS
codeblocks.

Additionally a new flag `--ext` was added to allowing specifying file extension
when formatting code from stdin. Available extensions are: `js`, `jsx`, `ts`,
`tsx`, `mjs`, `md`. Keep in mind that this flag has no effect when formatting
files on disk.
...
release we hde added a global TLS session cache which allows to reuse existing
TLS sessions between the connection.
...
pending reads would be cancel when closing a resource, while pending writes will
be able to complete.
...
- link to survey [https://forms.gle/hbhP46LUAfVFMggU6]
- link to 2020 recap post [https://deno.land/posts/deno-in-2020]
