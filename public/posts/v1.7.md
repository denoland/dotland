...
Deno! It takes only 10 minutes and will help us tremendously in further
development of Deno.
...
`Deno.shutdown()` is stabilized in this release. This function lets you shut
down a socket cleanly (so flushing all data before closing). This can be called
instead of `Deno.close` for supported sockets. Unlike the unstable version of
this API, no socket mode has to be specified when shutting down, as you only can
only really shut down the write end of a socket.
