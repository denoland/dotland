...
in stable supported architecture (Windows x86, MacOS x86, and Linux x86) to any
...
binaries that are 40-60% smaller than in 1.6.x. To try out this feature, use the
`--lite` flag when compiling your application. This tells `deno compile` to use
a slimmed down runtime-only Deno binary instead of the full Deno binary that is
used now.
...
Here is an example where we create a executable from the std/http/file_server
...
DNS over HTTPS nor DNS over TLS). One can specify a custom nameserver (for
...
We have replace three unstable APIs (`Deno.transpileOnly`, `Deno.bundle`, and
`Deno.compile`) for interacting with Deno's built-in TypeScript compiler with a
single improved function (`Deno.emit`). You can read all about how to use
...
well as collect coverage for Deno subprocesses spawned from tests. We'll
...
scratch, adding the ability to queue different "ops" acting on resources; this
means that `write`s to the same socket or file are now guaranteed to happen in
the same order as they were started, and so do `read`s.
