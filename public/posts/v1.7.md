...
Some servers requires users to reuse existing TLS sessions. Until now Deno
couldn't do it and established a new TLS session for each connection. In this
release we added a global TLS session cache which allows to reuse existing TLS
sessions between the connection.
...
[a blog post reviewing Deno's development in 2020](https://deno.land/posts/deno-in-2020),
we highly encourage you read it if you're interested in learning more about
Deno's journey last year.

Last but not least we would greatly appreciate if you filled
[first Deno survey](https://forms.gle/hbhP46LUAfVFMggU6), it takes only 10
minutes, but will help us tremendously in further development of Deno.
